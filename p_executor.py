...
def execute(domain_name,
    ...
    # instance_seed is the instance's block base (n*SEED_BLOCK), already resolved by the
    # caller. The env reset (slips) uses the base directly (slot 0); the fault-firing RNG
    # below uses a disjoint slot (base + fault_seed_offset) so the two streams never coincide.
    # Legacy callers leave fault_seed_offset=0 (historical single-stream behaviour preserved).
    ...
    obs = initial_obs
    rng = random.Random(instance_seed + fault_seed_offset)
